...
public class DNSservice extends Thread  {
	...
	public DNSservice(DatagramSocket server) {
			this.server=server;			
	...
	public void run() {
		...
		while (true) {
			...
			System.out.printf("Received packet from client %s:%s with length %s with the body:\n%s\n",
				...
				for (int i = 0; i < DNSlist.length; i++) {
					if (DNSlist[i][0].equals(domain_name)) {
						...
						found = true;
						service = "Name to IP";
					}else if(DNSlist[i][1].equals(domain_name)) {
						response = response + "IP: " + DNSlist[i][1] + "\tName: " + DNSlist[i][0] + "\n";
						found = true;
						service = "IP to Name";
					}
				}
				if (!found)
					response = response + "Could not resolve ... ";
				response = response + "\n";
			...
			Log.Log(String.valueOf(this.packet.getAddress()),this.startTime,this.duration,"DNS -> "+service);
		}
